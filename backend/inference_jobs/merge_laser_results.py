...
# ---- Filenames / paths ------------------------------------------------------
LASER_COMBINED_CSV_NAME = os.getenv("LASER_COMBINED_CSV", "laser_all_models.csv").strip()
LASER_COMPARE_NAME      = os.getenv("LASER_COMPARE_NAME", "classify/laser_compare.csv").strip()

LASER_SWINV2_CSV_NAME = os.getenv("LASER_SWINV2_CSV_NAME", "laser_swinv2_results.csv").strip()
LASER_MAXVIT_CSV_NAME = os.getenv("LASER_MAXVIT_CSV_NAME", "laser_maxvit_results.csv").strip()
LASER_COAT_CSV_NAME   = os.getenv("LASER_COAT_CSV_NAME",   "laser_coat_results.csv").strip()

COMPARE_PATH_OUT   = OUT_DIR / LASER_COMPARE_NAME
COMBINED_PATH_CSV  = CSV_DIR / LASER_COMBINED_CSV_NAME
COMBINED_PATH_OUT  = OUT_DIR / LASER_COMBINED_CSV_NAME

# ---- Helpers ----------------------------------------------------------------
def _read_csv_first(path: Path) -> List[Dict[str, str]]:
    """Read CSV if present; else []"""
    if not path.exists():
        return []
...
def _read_candidates(name: str) -> List[Dict[str, str]]:
    """Try OUT_DIR then CSV_DIR for per-model CSVs."""
...
def _pick_display_image(paths: List[str]) -> str:
    """Prefer a path that already points under regions/laser; else first non-empty; else basename."""
    for p in paths:
        if p and ("/regions/laser/" in p or "\\regions\\laser\\" in p):
            return p
    for p in paths:
        if p:
            return p
...
def run_merge(selected_models: Optional[List[str]] = None) -> Dict[str, str]:
    ...
    sw = _read_candidates(LASER_SWINV2_CSV_NAME) if want_swin else []
    mx = _read_candidates(LASER_MAXVIT_CSV_NAME) if want_max else []
    ct = _read_candidates(LASER_COAT_CSV_NAME)   if want_coat else []
    ...
    # Write compare + combined atomically (avoid partial/concatenated files)
